...
commonbtn.addEventListener("click", async ()=>{
    ...
    let listId = selected[0]._row.data.wishlist_id;
    let sql = "SELECT DISTINCT * from item i WHERE NOT EXISTS (select * FROM wishlist w WHERE w.user_id = "+listId+" AND NOT EXISTS (SELECT wi.item_id FROM wishlist_item wi WHERE w.wishlist_id = wi.list_id AND wi.item_id = i.item_id));"
    let res = await query(sql);
    mainTable.setData(res);
});
